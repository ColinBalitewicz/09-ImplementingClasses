...
class Point(object):
    def __init__(self,x,y):
        ...
        self.move=0
    def __repr__(self):
        return 'Point('+str(self.x)+' '+str(self.y)+') moved: '+str(self.move)

    def clone(self):
        return Point(self.x,self.y)

    def move_to(self,x,y):
        self.totaldistance=self.totaldistance+self.get_distance_from(Point(x,y))
        self.x=x
        self.y=y
    ...
    def move_by(self,dx,dy):
        self.totaldistance =self.totaldistance + self.get_distance_from(Point(self.x+dx,self.y+dy))
        self.x=self.x+dx
        self.y=self.y+dy
        self.move=self.move+1
    ...
    def get_distance_from_start(self):
        distance=((self.originalx-self.x)**2+(self.originaly-self.y)**2)**.5
        return distance
    ...
    def halfway_to(self,p2):
        p3=Point((self.x+p2.x)/2,(self.y+p2.y)/2)
        return p3
